...
def detecter_periode_saison(escales: pd.DataFrame) -> tuple[dt.date, dt.date]:
    """
    Détecte automatiquement les bornes de la saison à partir des dates
    d'arrivée présentes dans l'extraction (du 1er octobre au 30 septembre,
    saison à laquelle appartient la première date trouvée).
    """
    premiere_date = escales["ArriveeRade"].min().date()
    annee = premiere_date.year if premiere_date.month >= 10 else premiere_date.year - 1
    date_debut = dt.date(annee, 10, 1)
    date_fin = dt.date(annee + 1, 9, 30)
...
def transformer(fichier_csv, titre: str | None = None,
    ...
    return contenu_xlsx, len(tableau), postes_inconnus, date_debut, date_fin
